...
def go(args):
    ...
    ######################################
    # Use run.use_artifact(...).file() to get the train and validation
    # artifact (args.trainval_artifact) and save the returned path in
    # train_local_pat
    ...
    ######################################
    # Fit the pipeline sk_pipe by calling the
    # .fit method on X_train and y_train
    ...
    # add rf_config as metadata. Then, use the .add_dir method of the
    # artifact instance you just created to add the "random_forest_dir"
    # directory to the artifact,and finally use run.log_artifact
    # to log the artifact to the run
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    # in production (nor during training). That is not true for
    # neighbourhood_group
    ...
    # we create a feature that represents the number of days passed since
    # the last review. First we impute the missing review date with an old
    # date (because there hasn't been a review for a long time), and then we
    ...
    # a step called "preprocessor" applying the ColumnTransformer instance that
    # we saved in the `preprocessor` variable, and a step called
    # "random_forest" with the random forest instance that we just saved
